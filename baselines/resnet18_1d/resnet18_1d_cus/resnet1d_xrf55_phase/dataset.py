"""XRF55 raw phase dataset for ResNet1D-Phase baseline.

Loads pre-computed normalized raw phase files saved by scripts/local/01_preprocess.py:
  X_phase_raw_{split}.npy  shape (N, 180, 1000) — per-channel z-score (fit reps 1-12)
...
def _load_phase_raw(processed_dir: Path, split: str):
    p = Path(processed_dir)
    X = np.load(p / f'X_phase_raw_{split}.npy').astype(np.float32, copy=False)  # (N, 180, 1000)
    y = np.load(p / f'y_{split}.npy')
    return X, y


class XRF55PhaseRawDataset(Dataset):
...
def build_noval_loaders_with_val(processed_dir, batch_size=32, num_workers=4):
    ...
    """
    X_tr,  y_tr  = _load_phase_raw(processed_dir, 'train')
    X_val, y_val = _load_phase_raw(processed_dir, 'val')
    X_te,  y_te  = _load_phase_raw(processed_dir, 'test')
    kw = _kw(num_workers)
    train_loader = DataLoader(
        XRF55PhaseRawDataset(X_tr,  y_tr),
        batch_size=batch_size, shuffle=True, **kw)
    val_loader   = DataLoader(
        XRF55PhaseRawDataset(X_val, y_val),
        batch_size=64, shuffle=False, **kw)
    test_loader  = DataLoader(
        XRF55PhaseRawDataset(X_te,  y_te),
        batch_size=64, shuffle=False, **kw)
...
def build_loso_loaders_with_val(fold_idx: int, processed_dir,
    ...
    for split in ('train', 'val', 'test'):
        X, y    = _load_phase_raw(processed_dir, split)
    ...
    kw = _kw(num_workers)
    train_loader = DataLoader(
        XRF55PhaseRawDataset(np.concatenate(tr_X), np.concatenate(tr_y)),
        batch_size=batch_size, shuffle=True, **kw)
    val_loader   = DataLoader(
        XRF55PhaseRawDataset(np.concatenate(va_X), np.concatenate(va_y)),
        batch_size=64, shuffle=False, **kw)
    test_loader  = DataLoader(
        XRF55PhaseRawDataset(np.concatenate(te_X), np.concatenate(te_y)),
        batch_size=64, shuffle=False, **kw)
    return train_loader, val_loader, test_loader
