"""Train TF-Mamba-XRF55-Phase — multi-protocol, multi-seed, resume-safe.
...
Usage:
    cd har_csi
    python baselines/tf_mamba_xrf55_phase/trainer.py --protocol split --n_seeds 1
    python baselines/tf_mamba_xrf55_phase/trainer.py --protocol loso  --n_seeds 3
"""
import sys
...
from baselines.tf_mamba_xrf55_phase.dataset import (
    build_noval_loaders_with_val, build_loso_loaders_with_val,
)
from baselines.tf_mamba_xrf55_phase.model import TFMamba
...
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'runs' / 'tf_mamba_xrf55_phase'

D_MODEL     = 64
NUM_LAYERS  = 3
NUM_CLASSES = 11
XH_FEATURES = 90    # features per token in XH stream (last dim)
...
XV_SEQ_LEN  = 90    # seq_len of XV stream (axis 1) — used by positional embedding
...
CONFIG = ExperimentConfig(
    model_name='tf_mamba_xrf55_phase',
