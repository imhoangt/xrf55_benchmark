"""XRF55 phase dataset for TF-Mamba-XRF55-Phase baseline.

Loads pre-computed Haar DWT files saved by scripts/local/01_preprocess.py:
  X_phase_xh_{split}.npy  shape (N, 500, 90) — Time-Mamba stream (per-channel z-score)
  X_phase_xv_{split}.npy  shape (N, 90, 500) — Freq-Mamba stream (per-channel z-score)
...
def _load_phase(processed_dir: Path, split: str):
    """Load pre-computed Haar DWT phase split."""
    p = Path(processed_dir)
    XH = np.load(p / f'X_phase_xh_{split}.npy').astype(np.float32, copy=False)  # (N, 500, 90)
    XV = np.load(p / f'X_phase_xv_{split}.npy').astype(np.float32, copy=False)  # (N, 90, 500)
    y  = np.load(p / f'y_{split}.npy')
    return XH, XV, y


class XRF55PhaseHaarDatasetFromArrays(Dataset):
...
def build_noval_loaders_with_val(processed_dir, batch_size=32, num_workers=4):
    ...
    """
    XH_tr,  XV_tr,  y_tr  = _load_phase(processed_dir, 'train')
    XH_val, XV_val, y_val = _load_phase(processed_dir, 'val')
    XH_te,  XV_te,  y_te  = _load_phase(processed_dir, 'test')
    kw = _kw(num_workers)
    train_loader = DataLoader(
        XRF55PhaseHaarDatasetFromArrays(XH_tr,  XV_tr,  y_tr),
        batch_size=batch_size, shuffle=True, **kw)
    val_loader   = DataLoader(
        XRF55PhaseHaarDatasetFromArrays(XH_val, XV_val, y_val),
        batch_size=64, shuffle=False, **kw)
    test_loader  = DataLoader(
        XRF55PhaseHaarDatasetFromArrays(XH_te,  XV_te,  y_te),
        batch_size=64, shuffle=False, **kw)
...
def build_loso_loaders_with_val(fold_idx: int, processed_dir,
    ...
    for split in ('train', 'val', 'test'):
        XH, XV, y = _load_phase(processed_dir, split)
    ...
    kw = _kw(num_workers)
    train_loader = DataLoader(
        XRF55PhaseHaarDatasetFromArrays(
