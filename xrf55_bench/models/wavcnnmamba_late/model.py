...
Mamba backbone. ~0.72M params in the default config.
...
Per-subband path (LATE fusion):
    subband_s (B, M*A, T2, F2)
      ...
      -> TFBlock x3    depthwise axial (dil 1/2/4)             (B, 128, T2, F2)
      -> FreqAttnPool  4-head, collapse F                      (B, T2, 128)
      -> [PosEmb]      learned, optional (default OFF)         (B, T2, 128)
      -> BiMamba x2    fwd+bwd, no FFN, d_state=32             (B, T2, 128)
...
# ─── CNN block: pre-norm axial depthwise (residual) ──────────────────────────

class TFBlock(nn.Module):
    """Parallel axial depthwise mixing along time (dilated) and frequency,
    a pointwise channel projection, and a residual connection.
...
class FreqAttnPool(nn.Module):
    ...
    Default n_heads=4: disjoint head groups, each softmaxes over F independently,
    plus an identity-initialised out_proj for cross-head mixing.
    n_heads=1 gives a single-head pool with no out_proj (lighter baseline).
...
class SharedBackbone(nn.Module):
    ...
    def __init__(self, d_stem: int = 32, d_model: int = 128,
                 ...
                 n_freq_heads: int = 4, n_mamba_layers: int = 2,
                 d_state: int = 32, dp_mamba=(0.0, 0.05),
...
class WavMambaHAR(nn.Module):
    ...
    Args:
        ...
        n_freq_heads   : FreqAttnPool heads (default 4).
        ...
        stem_kernels   : per-subband (k_t, k_f) tuples; default LL/HL/LH kernels
                         for n_subbands==3, else (5,5) each.
    ...
    def __init__(
        ...
        self.backbone = SharedBackbone(
            d_stem=d_stem, d_model=d_model, dp_cnn=dp_cnn,
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if self.fusion == "late":
            subs = X.chunk(self.n_subbands, dim=1)
            vecs = [self.backbone(stem(sb))
