"""Train and evaluate TF-Mamba on NTU-Fi.

Preprocessing: CSI_Dataset from original paper — loadmat CSIamp, fixed normalization
(x-42.3199)/4.9802, downsample 2000->500, reshape (3,114,500), flatten (342,500).
DWT on (N,342,500) → XH(250,171), XV(171,250).
...
Usage:
    cd har_csi
    python baselines/tf_mamba/tf_mamba_original/tf_mamba_ntu_fi/trainer.py

Outputs saved to outputs/runs/tf_mamba_ntufi/:
...
from baselines.tf_mamba.tf_mamba_original.tf_mamba_ntu_fi.dataset import NUM_CLASSES, build_loaders
from baselines.tf_mamba.tf_mamba_original.tf_mamba_ntu_fi.model import TFMamba
from src.training.amp_utils import torch_load_checkpoint
from src.training.train_utils import set_seed

# ── Config ────────────────────────────────────────────────────────────────────

DATA_ROOT       = PROJECT_ROOT / 'dataset' / 'NTU-Fi'
OUTPUT_DIR      = PROJECT_ROOT / 'outputs' / 'runs' / 'tf_mamba_ntufi'
...
D_MODEL      = 64
NUM_LAYERS   = 3
NUM_FEATURES = 171    # M = 342/2 = 171 after DWT
MAX_LEN      = 250    # L = 500/2 = 250 after DWT
